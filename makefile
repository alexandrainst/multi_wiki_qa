...
install: ## Install dependencies
	@echo "Installing the 'multi_wiki_qa' project..."
	...
	@echo "Installed the 'multi_wiki_qa' project! You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:alexandrainst/multi_wiki_qa.git; \
	fi
...
publish-docs:  ## Publish documentation to GitHub Pages
	...
	@echo "Updated documentation website: https://alexandrainst.github.io/multi_wiki_qa"

test:  ## Run tests
	@uv run pytest && uv run readme-cov

docker:  ## Build Docker image and run container
	@docker build -t multi_wiki_qa .
	@docker run -it --rm multi_wiki_qa
